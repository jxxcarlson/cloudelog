...
**Cabal note:** `backend/cloudelog-backend.cabal` is gitignored and regenerated by hpack from `backend/package.yaml` on every build. `package.yaml` uses hpack auto-discovery, so dropping new `.hs` files under `backend/src/` and `backend/test/` is sufficient — no explicit `exposed-modules` edit is needed. Steps below that say "add module to cabal" refer to this automatic behavior.
...
- [ ] **Step 1: (no-op under hpack auto-discovery)**

See the "Cabal note" in the File map section. Dropping the new `.hs` files under `backend/src/Service/` and `backend/test/Service/` in later steps is sufficient — hpack will pick them up on the next build.
...
See the "Cabal note" in the File map — creating `backend/src/Db/Streak.hs` in Step 2 is sufficient for hpack to pick it up.
...
```
git add backend/src/Db/Streak.hs
